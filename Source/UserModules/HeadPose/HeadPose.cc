...
namespace headpose 
{
    ...
    void drawCylinder( const math_vector_3f& p1, const math_vector_3f& p2 , float radius, GLUquadric *quadric)
    {
	    ...
	    float n = length(d);
	    float ax = ( d[2] < 0.0 ) ? -57.295779f*ikaros::acos( d[2]/n ) : 57.295779f*ikaros::acos( d[2]/n );
...
HeadPose::Init()
{
	...
	glutDisplayFunc(headpose::draw);
	glutMouseFunc(headpose::mb);
	glutMotionFunc(headpose::mm);
	glutKeyboardFunc(headpose::key);
	glutReshapeFunc(headpose::resize);
	glutIdleFunc(headpose::idle);
	glutMainLoop();
}
